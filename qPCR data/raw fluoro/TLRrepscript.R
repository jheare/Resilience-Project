...
rep1<-read.csv("TLR3rawfluoro.csv", header = T)
...
write.csv(rep1res2, file="TLRrep1res2.csv")
rep1edit<-read.csv("TLRrep1res2.csv",header=T)
...
rep1edit$sig.eff<-as.numeric(as.character(rep1edit$sig.eff))
rep1edit$sig.cpD2<-as.numeric(as.character(rep1edit$sig.cpD2))

#Now I plot the Ct values to see how they align without converting them to expression.
ggplot(rep1edit, aes(x=Names,y=sig.cpD2, fill=Pop))+geom_bar(stat="identity")
...
rep1edit$expression<-expr(rep1edit$sig.eff, rep1edit$sig.cpD2)

#Graphing the expression values is a good way to examine the data quickly for errors that might have occurred. 
ggplot(rep1edit, aes(x=Names,y=expression, fill=Pop))+geom_bar(stat="identity")
...
rep2<-read.csv("TLR4rawfluoro.csv", header = T)
rep2$X<-NULL
...
write.csv(rep2res2, file="TLRrep2res2.csv")
rep2edit<-read.csv("TLRrep2res2.csv",header=T)
rep2edit$sig.eff<-as.numeric(as.character(rep2edit$sig.eff))
rep2edit$sig.cpD2<-as.numeric(as.character(rep2edit$sig.cpD2))


ggplot(rep2edit, aes(x=Names,y=sig.cpD2, fill=Pop))+geom_bar(stat="identity")
...
rep2edit$expression<-expr(rep2edit$sig.eff, rep2edit$sig.cpD2)


ggplot(rep2edit, aes(x=Names,y=expression, fill=Pop))+geom_bar(stat="identity")
...
repcomp<-as.data.frame(rep1edit$sig.cpD2-rep2edit$sig.cpD2)
...
repcomp<-rename(repcomp, c("rep1edit$sig.cpD2 - rep2edit$sig.cpD2"="rep.diff", "Names_1"="Pop", "Names_2"="Treat", "Names_3"="Sample"))
...
tlr<-as.data.frame(cbind(rep1edit$expression,as.character(rep1edit$Names),as.character(rep1edit$Pop),as.character(rep1edit$Treat),rep2edit$expression))
